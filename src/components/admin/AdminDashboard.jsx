...
function AdminDashboard({ onLogout, onNavigateHome }) {
  ...
  // Helper to normalize specs into [{ label, value }] safely
    ...
    if (Array.isArray(specs)) {
      return specs.map(s => {
        ...
        if (s && typeof s === 'object') {
          return {
            label: s.label || s.name || s.key || 'Spesifikasi',
            value: s.value || s.desc || s.val || '-'
          };
        }
              ...
              <button
                onClick={() => {
                  setEditingProduct({
                    ...
                    gallery: [],
                    specs: [
                      { label: 'Kapasitas', value: '8 - 12 m³' },
                      { label: 'Sasis Cocok', value: 'Isuzu Elf NMR / Hino Dutro / Canter' },
                      { label: 'Material Bak', value: 'Baja Plat SS400 / Q345B' },
                      { label: 'Sistem Hidrolik', value: 'Telescopic Cylinder Heavy-Duty' },
                      { label: 'Finishing', value: 'Epoxy Primer + Polyurethane Topcoat' },
                    ],
                    ...
                    <div className="p-4 sm:p-5">
                      ...
                      {prod.specs && (
                        <div className="mt-3 pt-3 border-t border-zinc-800/80 grid grid-cols-2 gap-2 text-[11px]">
                          {normalizeSpecs(prod.specs).slice(0, 2).map((s, idx) => (
                            <div key={idx} className="bg-zinc-950/60 p-2 rounded-lg">
                              <span className="text-zinc-500 block truncate">{s.label}</span>
                              <span className="text-zinc-200 font-semibold truncate block">{s.value}</span>
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  <form onSubmit={handleSaveProduct} className="space-y-5 text-xs">
                    ...
                    <div className="bg-zinc-950/80 border border-zinc-800 rounded-xl p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (newKey) {
                              const current = normalizeSpecs(editingProduct.specs);
                              setEditingProduct({
                                ...editingProduct,
                                specs: [...current, { label: newKey, value: '-' }],
                              });
                            }
                          }}
                      ...
                      <div className="space-y-2">
                        {normalizeSpecs(editingProduct.specs).map((item, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const current = normalizeSpecs(editingProduct.specs).filter((_, i) => i !== idx);
                                setEditingProduct({ ...editingProduct, specs: current });
                              }}
...
class AdminErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('Admin Dashboard Caught Exception:', error, errorInfo);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-zinc-950 text-white flex flex-col items-center justify-center p-6 text-center font-sans">
          <div className="p-6 bg-zinc-900 border border-amber-500/40 rounded-2xl max-w-md shadow-2xl space-y-4">
            ...
            <h3 className="text-base font-extrabold uppercase text-white">
              Pembaruan Tampilan Sedang Dimuat
            </h3>
            <p className="text-xs text-zinc-400 leading-relaxed">
              {this.state.error?.message || 'Komponen berhasil dipulihkan. Klik tombol di bawah untuk menyegarkan tampilan.'}
            </p>
            <button
              onClick={() => {
              ...
              className="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-black uppercase text-xs rounded-xl shadow-lg transition"
            >
              Segarkan Dashboard
            </button>
          </div>
