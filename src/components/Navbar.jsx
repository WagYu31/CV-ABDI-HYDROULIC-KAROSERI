...
import { PhoneCall, Menu, X, ChevronRight, FileText } from 'lucide-react';
    ...
    <>
      ...
      <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-xl border-b border-slate-200/90 py-3 shadow-xl shadow-slate-900/5' : 'bg-transparent py-4'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          {/* Desktop Navigation Links & PDF Action */}
          <div className="hidden lg:flex items-center gap-3">
            <nav className="flex items-center gap-1 bg-white/90 p-1.5 rounded-full border border-slate-200 shadow-sm backdrop-blur-md">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-amber-500/15 hover:bg-amber-500/25 text-amber-950 border border-amber-500/40 text-xs font-mono font-bold transition-all shadow-xs cursor-pointer"
              title="Baca & Unduh Dokumen Resmi Company Profile (PDF 13 MB)"
            >
          ...
          >
            ...
            >
              <button
                ...
                className="w-full py-3 rounded-xl text-center text-sm font-bold bg-amber-500 hover:bg-amber-400 text-slate-950 flex items-center justify-center gap-2 shadow-md font-mono transition-colors"
              >
